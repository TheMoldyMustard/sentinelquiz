...
export default function MultipleSelectMode({
  item,
  choices,
  hintUsed,
  onSubmit,
  ...
  const handleHint = () => {
    ...
    const wrong = choices.filter(
      (choice) => !correctChoices.has(normalizeAnswer(choice)) && !eliminated.has(choice)
    );
    if (wrong.length > 0) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <span className="tag">
          <span className="text-sentinel-warn">■</span> MULTIPLE SELECT
        </span>
        <span className="tag text-sentinel-text-dim">x0.75 MULTIPLIER</span>
      </div>
        ...
        {choices.map((choice, idx) => {
          ...
          return (
            <motion.button
              ...
              onClick={() => !isEliminated && !disabled && toggleChoice(choice)}
      ...
      <div className="flex gap-3">
        ...
        {!hintUsed && (
          <button onClick={handleHint} disabled={disabled} className="btn-ghost">
            ELIMINATE (-25pts)
          </button>
        )}
      </div>
